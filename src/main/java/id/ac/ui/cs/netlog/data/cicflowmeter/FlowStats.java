...
public class FlowStats {
    ...
    // Subflow Statistics
    private long subflowFwdPackets;     // Subflow Forward Packets
    private long subflowFwdBytes;       // Subflow Forward Bytes
    private long subflowBwdPackets;     // Subflow Backward Packets
    private long subflowBwdBytes;       // Subflow Backward Bytes
    ...
    public FlowStats(Flow flow) {
        ...
        this.protocol = flow.getProtocol();
        
        // Timestamp
        this.timestamp = DateUtils.convertMilliseconds2String(flow.getFlowStartTime()/1000L, "dd/MM/yyyy hh:mm:ss a");
        ...
        // Flow rates
        double durationInSeconds = (double)flowDuration/1000000L;
        this.flowBytesPerSec = ((double)(flow.getForwardBytes() + flow.getBackwardBytes())) / durationInSeconds;
        this.flowPacketsPerSec = ((double)flow.packetCount()) / durationInSeconds;
        
        // Flow IAT
        this.flowIatMean = flow.getFlowIAT().getMean();
        this.flowIatStd = flow.getFlowIAT().getStandardDeviation();
        this.flowIatMax = flow.getFlowIAT().getMax();
        this.flowIatMin = flow.getFlowIAT().getMin();
